...
namespace swr {
    std::string getRequest(std::string site, std::string param)
    {
    ...
    bool downloadRequest(std::string site, std::string filename)
    {
        if (URLDownloadToFileA(NULL, site.c_str(), filename.c_str(), BINDF_GETNEWESTVERSION, NULL))
            return true;
        else
            return false;
    }
